...
/**
 * BlackHoleBackground — fixed, full-viewport 3D accretion-disk animation.
 * Adapted from an Originkit "Black Hole" component (originally built for Framer)
 * to run as a plain React background layer: TypeScript types stripped, Framer
 * property-control scaffolding removed, and the container background made
 * transparent so the site's own light/dark theme color shows through — only
 * the event horizon itself stays black, since that's what a black hole is.
 */

const BG = "#000000"; // event-horizon fill only — NOT the page background
const PERSPECTIVE = 1300;

const DEFAULT_CENTRE = { voidRadius: 55, voidX: 50, voidY: 50 };
...
export default function BlackHoleBackground(props = {}) {
  const {
  ...
  // The original component was built for a fixed 600px canvas, where a
  // ~55px void radius reads as a proportionate event horizon. Used as-is on
  // a full-viewport background it's a tiny dot lost in the middle of the
  // screen — so scale it relative to the smaller viewport dimension instead,
  // using 600px as the reference size the raw value was designed against.
  const REFERENCE_SIZE = 600;
  const getVoidRadius = useCallback(
    (w, h) => {
      if (showCenter === false) return 1;
      const scale = Math.min(w, h) / REFERENCE_SIZE;
      return Math.max(8, rawVoidRadius * scale);
    },
    [rawVoidRadius, showCenter]
  );
  ...
  useEffect(() => {
    ...
    const vr = getVoidRadius(w, h);
    initParticles(particleCount, vr, outerRadFromSize(w, h, vr), colors.length);
  }, [particleCount, getVoidRadius, colors.length, initParticles, outerRadFromSize, sizeVersion]);
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      const voidRadius = getVoidRadius(w, h);
      const outerRad = outerRadFromSize(w, h, voidRadius);
